...
public sealed class PostgreSqlPayrollRunsAcceptanceTests
{
    ...
    public async Task ConfigureAsync()
    {
        ...
        var tenant = Tenant.CreateForGrower(growerId, label, null); var farm = tenant.CreateFarm($"P6B{Guid.NewGuid():N}"[..20], label, "Synthetic address", "Railway Development", "Synthetic", 10m, "Synthetic"); var manager = farm.AddPerson("Synthetic P6B manager", null, new DateOnly(2026, 1, 1)); farm.AssignRole(manager, PersonRole.FarmManager, true, new DateOnly(2026, 1, 1)); tenant.AddMembership(_managerUserId, manager.Id, TenantSecurityRoles.FarmManager);
