...
public sealed class PostgreSqlStockCountAdjustmentAcceptanceTests
{
    ...
    {
        var label = $"{_runId}-{Guid.NewGuid():N}"; var grower = $"p5d-grower-{Guid.NewGuid():N}"; var manager = $"p5d-manager-{Guid.NewGuid():N}"; var tenant = Tenant.CreateForGrower(grower, label, null); var farm = tenant.CreateFarm($"F{Guid.NewGuid():N}"[..20], label, "Synthetic", "Railway", "Synthetic", 1m, "Synthetic"); tenant.AddMembership(manager, farm.AddPerson("Synthetic manager", null, new DateOnly(2026, 1, 1)).Id, TenantSecurityRoles.FarmManager); var unit = UnitOfMeasure.Create(tenant.Id, $"U{Guid.NewGuid():N}"[..20], "Unit", "Mass", 6); var item = InventoryItem.Create(tenant.Id, farm.Id, $"I{Guid.NewGuid():N}"[..20], label, InventoryItemCategory.Other, unit, null, LotTrackingPolicy.None, ExpiryPolicy.None); var position = StockPosition.Create(tenant.Id, farm.Id, farm.Store.Id, item.Id, null); var supplier = Supplier.Create(tenant.Id, farm.Id, $"S{Guid.NewGuid():N}"[..20], label, null); StockReceipt? receipt = null; StockReceiptLine? line = null; if (quantity > 0) { receipt = StockReceipt.Create(tenant.Id, farm.Id, farm.Store.Id, StockReceiptType.Purchase, supplier.Id, new DateOnly(2026, 8, 26), null, label, null, null, 0); line = receipt.AddLine(item, null, quantity, cost, receipt.Version); receipt.MarkPosted(DateTimeOffset.UtcNow, grower, $"{label}-posted", receipt.Version); }
