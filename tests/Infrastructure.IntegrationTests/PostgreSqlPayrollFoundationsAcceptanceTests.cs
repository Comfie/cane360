...
public sealed class PostgreSqlPayrollFoundationsAcceptanceTests
{
    ...
    private async Task<Scenario> CreateScenarioAsync(bool approved = false)
    {
        ...
        var manager = farm.AddPerson("Synthetic manager", null, new DateOnly(2026, 1, 1)); farm.AssignRole(manager, PersonRole.FarmManager, true, new DateOnly(2026, 1, 1)); tenant.AddMembership(managerId, manager.Id, TenantSecurityRoles.FarmManager); var workerPerson = farm.AddPerson("Synthetic worker", null, new DateOnly(2026, 1, 1));
