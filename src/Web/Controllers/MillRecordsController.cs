...
public sealed class MillRecordsController(IMillRecordsService records) : ControllerBase
{
    ...
    {
        return Ok(await records.GetSessionAsync(cancellationToken));
    }
    ...
    {
        return Ok(await records.GetMillsAsync(includeInactive, cancellationToken));
    }
    ...
    public async Task<ActionResult<MillDto>> UpdateMill(Guid millId, MillRequest request,
        CancellationToken cancellationToken)
    {
        return Ok(await records.UpdateMillAsync(millId,
            new MillInput(request.Code, request.Name, request.Location, request.ExpectedVersion), cancellationToken));
    }
    ...
    public async Task<ActionResult<MillDto>> DeactivateMill(Guid millId,
        RecordMillRecordRequest request, CancellationToken cancellationToken)
    {
        return Ok(await
            records.DeactivateMillAsync(millId, new VersionedInput(request.ExpectedVersion), cancellationToken));
    }

    [HttpPost("mills/{millId:guid}/reactivate", Name = "ReactivateMill")]
    public async Task<ActionResult<MillDto>> ReactivateMill(Guid millId,
        RecordMillRecordRequest request, CancellationToken cancellationToken)
    {
        return Ok(await
            records.ReactivateMillAsync(millId, new VersionedInput(request.ExpectedVersion), cancellationToken));
    }
    ...
    public async Task<ActionResult<WeighbridgeTicketDto>> GetTicket(Guid ticketId,
        CancellationToken cancellationToken)
    {
        return Ok(await records.GetTicketAsync(ticketId, cancellationToken));
    }
    ...
    {
        return Ok(await
            records.RecordTicketAsync(ticketId, new RecordInput(request.ExpectedVersion, request.IdempotencyKey),
                cancellationToken));
    }
    ...
    {
        if (!TryEvidence(request, out byte[] content))
        {
            return EvidenceError();
        }

        await using MemoryStream stream = new(content, false);
        return Ok(await records.UploadTicketEvidenceAsync(ticketId, new EvidenceUpload(stream, request.FileName,
    ...
    public async Task<ActionResult<GrowerStatementDto>> GetStatement(Guid statementId,
        CancellationToken cancellationToken)
    {
        return Ok(await records.GetStatementAsync(statementId,
            cancellationToken));
    }
    ...
    {
        return Ok(await
            records.RecordStatementAsync(statementId, new RecordInput(request.ExpectedVersion, request.IdempotencyKey),
                cancellationToken));
    }
        ...
        EvidenceUploadRequest request, CancellationToken cancellationToken)
    {
        ...
        await using MemoryStream stream = new(content, false);
        return Ok(await records.UploadStatementEvidenceAsync(statementId, new EvidenceUpload(stream,
    ...
    public async Task<ActionResult<IReadOnlyList<CandidateTicketDto>>> GetCandidates(Guid statementId,
        CancellationToken cancellationToken)
    {
        return Ok(await records.GetCandidateTicketsAsync(statementId,
            cancellationToken));
    }
    ...
    public async Task<ActionResult<ReconciliationSummaryDto>> AddMatch(Guid statementId,
        AddStatementTicketMatchRequest request, CancellationToken cancellationToken)
    {
        return Ok(await
            records.AddMatchAsync(statementId, new AddMatchInput(request.WeighbridgeTicketId, request.MatchedTonnes,
    ...
    public async Task<ActionResult<ReconciliationSummaryDto>> ReverseMatch(Guid statementId,
        ...
        CancellationToken cancellationToken)
    {
        return Ok(await records.ReverseMatchAsync(statementId,
            matchId, new ReverseMatchInput(request.Reason, request.IdempotencyKey), cancellationToken));
    }
    ...
    public async Task<ActionResult<ReconciliationSummaryDto>> GetReconciliation(Guid statementId,
        CancellationToken cancellationToken)
    {
        return Ok(await records.GetReconciliationAsync(statementId,
            cancellationToken));
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        ReportExportContext export = await records.RecordExportAsync("WeighbridgeRegister",
            ...
            cancellationToken);
        StringBuilder csv = ExportHeader(export);
        csv.AppendLine("Mill,Ticket,Date,Net tonnes,Field,Crop cycle,Match state");
        foreach (WeighbridgeTicketDto row in rows.Where(x => x.IsCurrent))
        {
            csv.AppendLine(string.Join(',',
                Csv(row.MillName), Csv(row.TicketReference), Csv(row.TicketDate), CsvCell.Number(row.NetTonnes),
                Csv(row.FieldName), Csv(row.CropCycleLabel),
                row.MatchedStatementIds.Count == 0 ? "Unmatched" : "Matched"));
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        StringBuilder csv = ExportHeader(export);
        csv.AppendLine(
            "Statement,Period start,Period end,Mill,Statement tonnes,Matched tonnes,Tonnes variance,Statement USD,Matched USD,Amount variance,Status");
        foreach (GrowerStatementDto row in rows.Where(x => x.IsCurrent))
        {
            csv.AppendLine(string.Join(',',
                Csv(row.StatementReference), Csv(row.PeriodStart), Csv(row.PeriodEnd), Csv(row.MillName),
                CsvCell.Number(row.TotalTonnes), CsvCell.Number(row.Reconciliation.MatchedTicketTonnes),
                CsvCell.Number(row.Reconciliation.TonnesVariance),
    ...
    private static StringBuilder ExportHeader(ReportExportContext context)
    {
        return new StringBuilder()
            .AppendLine($"Report,{Csv(context.Report)}")
            .AppendLine($"Farm,{Csv(context.Farm)}")
            .AppendLine(
                $"Filters,{Csv(string.IsNullOrEmpty(context.Filters) ? "All authorized current records; all dates" : context.Filters)}")
            .AppendLine(
                $"Generated UTC,{Csv(context.GeneratedAt.ToString("O", CultureInfo.InvariantCulture))}")
            .AppendLine($"Source,{Csv(context.Source)}");
    }

    private static TicketInput TicketInput(TicketRequest request, DateOnly date)
    {
    ...
    private static StatementInput StatementInput(StatementRequest request, DateOnly start,
        DateOnly end)
    {
        return new StatementInput(request.MillId, request.StatementReference, start, end,
        ...
        out DateOnly? toDate)
    {
        ...
        {
            if (!TransportValueParser.TryParseDateOnly(from, out DateOnly parsed))
            {
                return false;
            }
        ...
        {
            if (!TransportValueParser.TryParseDateOnly(to, out DateOnly parsed))
            {
                return false;
            }
    ...
    private static bool TryEvidence(EvidenceUploadRequest request, out byte[] content)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.FileName) || string.IsNullOrWhiteSpace(request.ContentType) ||
            string.IsNullOrWhiteSpace(request.ContentBase64) || request.ContentBase64.Length > 28_000_000)
        {
            return false;
        }
