...
import { dateOnly, employmentTypes, evidenceAmount, harareToday, label, payBases } from '../labour/labourView';
...
function EvidenceLedger({ date, register, references, records, onChanged, onError }) {
  ...
  /** @param {import('react').FormEvent<HTMLFormElement>} event */
  const submit = async (event) => {
    event.preventDefault(); const data = new FormData(event.currentTarget); const activityIds = data.getAll('activityIds').map(String); const scopeType = String(data.get('scopeType') || ''); setSaving(true); onError('');
