...
export function PayrollPage() {
  ...
  const reloadCore = useCallback(async () => {
    const [nextWorkspace, nextPeriods, nextAdvances, nextRuns] = await Promise.all([api.workspace(), api.getPeriodsPayroll(), api.getAdvancesPayroll(), api.getRunsPayroll()]);
  ...
  useEffect(() => {
    ...
    api.runs(selectedRunId).then((detail) => {
  ...
  const createPeriod = async (event: FormEvent<HTMLFormElement>) => {
    ...
    await runMutation('period-create', () => api.createPeriodPayroll(new CreatePayrollPeriodRequest(periodPayload(String(data.get('year') ?? ''), String(data.get('month') ?? '')))), 'Draft payroll period created.');
  };
  ...
  const cancelPeriod = async (event: FormEvent<HTMLFormElement>, period: PayrollPeriodDto) => {
    ...
    if (await runMutation(`period-cancel-${period.id}`, () => api.cancelPeriodPayroll(period.id, new CancelPayrollPeriodRequest({ expectedVersion: period.version, reason })), 'Draft payroll period cancelled.')) setPeriodCancelId('');
  };
  ...
  const saveAdvance = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const action = editingAdvance
      ? () => api.updateWorkerAdvance(editingAdvance.id, new UpdateWorkerAdvanceRequest({ amountUsd: Number(advanceForm.amountUsd), reason: advanceForm.reason.trim(), requestedEventDate: advanceForm.requestedEventDate, recoveryStartPayrollPeriodId: advanceForm.recoveryStartPayrollPeriodId, installmentCount: Number(advanceForm.installmentCount), expectedVersion: editingAdvance.expectedVersion }))
      : () => api.createAdvancePayroll(new CreateWorkerAdvanceRequest(payload));
  ...
  const decideAdvance = async (advance: WorkerAdvanceDto, approved: boolean) => {
    ...
    await runMutation(`advance-decision-${advance.id}`, () => api.decideAdvancePayroll(advance.id, new DecideWorkerAdvanceRequest({ expectedVersion: advance.version, approved, reason: approved ? undefined : decisionReason.trim(), idempotencyKey: decisionKeys.current.get(keyName) ?? '' })), approved ? 'Advance approved. No money has been issued.' : 'Advance rejected with an immutable decision record.');
    setDecisionReason('');
  };
  ...
  const createRun = async () => {
    const created = await runMutation('run-create', async () => { const result = await api.createRunPayroll(new CreatePayrollRunRequest({ payrollPeriodId: selectedPeriodId })); setSelectedRunId(result.id); }, 'Payroll run created. Calculate it from authoritative labour evidence.');
    return created;
  };
  ...
  const decideRun = async (approved: boolean) => {
    ...
    await runMutation(`run-decision-${selectedRun.id}`, () => api.decideRunPayroll(selectedRun.id, new DecidePayrollRunRequest(payload)), approved ? 'Payroll approved and period closed. No money was paid.' : 'Payroll rejected without consuming evidence or recovering advances.');
  ...
  return <section className="record-panel payroll-run-panel">
    ...
    {runs.length === 0 ? <EmptyState title="No payroll runs" copy={selectedPeriod?.status === 'Open' ? 'Create the single payroll run for this open period.' : 'Open a payroll period, then create its run.'} /> : <div className="payroll-run-workspace">
      ...
      {selectedRun && <article className="payroll-run-detail" aria-busy={detailLoading}>
        ...
        {selectedRun.status === 'Rejected' && <div className="payroll-warning"><CircleAlert size={17} /><span><strong>Grower rejected calculation v{selectedRun.submittedCalculationVersion}</strong>{selectedRun.rejectionReason}</span></div>}
