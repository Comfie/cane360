...
export function PayrollPage() {
  ...
  const cancelPeriod = async (event: FormEvent<HTMLFormElement>, period: PayrollPeriodDto) => {
    ...
    if (await runMutation(`period-cancel-${period.id}`, () => api.cancelPayrollPeriod(period.id, new CancelPayrollPeriodRequest({ expectedVersion: period.version, reason })), 'Draft payroll period cancelled.')) setPeriodCancelId('');
  };
  ...
  const decideAdvance = async (advance: WorkerAdvanceDto, approved: boolean) => {
    ...
    await runMutation(`advance-decision-${advance.id}`, () => api.decideWorkerAdvance(advance.id, new DecideWorkerAdvanceRequest({ expectedVersion: advance.version, approved, reason: approved ? undefined : decisionReason.trim(), idempotencyKey: decisionKeys.current.get(keyName) ?? '' })), approved ? 'Advance approved. No money has been issued.' : 'Advance rejected with an immutable decision record.');
    setDecisionReason('');
  };
  ...
  const decideRun = async (approved: boolean) => {
    ...
    await runMutation(`run-decision-${selectedRun.id}`, () => api.decidePayrollRun(selectedRun.id, new DecidePayrollRunRequest(payload)), approved ? 'Payroll approved and period closed. No money was paid.' : 'Payroll rejected without consuming evidence or recovering advances.');
