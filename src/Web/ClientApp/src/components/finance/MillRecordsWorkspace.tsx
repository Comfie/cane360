...
import { FileDown, FilePlus2, Link2, Pencil, Plus, Power, PowerOff, Printer, Scale, Warehouse, X } from 'lucide-react';
import {
...
export function MillRecordsWorkspace({ onError, onSuccess }: {
  ...
  return <div className="mill-workspace" aria-busy={refreshing}>
    ...
    {view === 'mills' && <section className="mill-reference-list record-panel">{mills.map((mill) => <article key={mill.id}><span><strong>{mill.code}</strong><small>{mill.name}{mill.location ? ` · ${mill.location}` : ''}</small></span><em className={`status-pill status-${mill.active ? 'active' : 'inactive'}`}>{mill.active ? 'Active' : 'Inactive'}</em><button type="button" className="row-icon-button" onClick={() => setMillEditor(mill)} aria-label={`Edit ${mill.name}`} title={`Edit ${mill.name}`}><Pencil size={15} /></button>{mill.active ? <button type="button" className="row-icon-button row-icon-button-danger" disabled={pending !== ''} onClick={() => mutate(`mill-${mill.id}`, () => api.deactivateMill(mill.id, new RecordMillRecordRequest({ expectedVersion: mill.version, idempotencyKey: newFinanceKey('mill-deactivate') })), 'Mill deactivated; historical evidence remains visible.')} aria-label={`Deactivate ${mill.name}`} title={`Deactivate ${mill.name}`}><PowerOff size={15} /></button> : <button type="button" className="row-icon-button" disabled={pending !== ''} onClick={() => mutate(`mill-${mill.id}`, () => api.reactivateMill(mill.id, new RecordMillRecordRequest({ expectedVersion: mill.version, idempotencyKey: newFinanceKey('mill-reactivate') })), 'Mill reactivated.')} aria-label={`Reactivate ${mill.name}`} title={`Reactivate ${mill.name}`}><Power size={15} /></button>}</article>)}</section>}
