...
internal sealed class PayrollWorkerLineConfiguration : IEntityTypeConfiguration<PayrollWorkerLine>
{
    public void Configure(EntityTypeBuilder<PayrollWorkerLine> builder)
    {
        ...
        builder.HasKey(x => x.Id); builder.Property(x => x.Id).ValueGeneratedNever(); builder.HasAlternateKey(x => new { x.Id, x.TenantId, x.FarmId }); builder.Property(x => x.WorkerNameSnapshot).HasMaxLength(200).IsRequired();
