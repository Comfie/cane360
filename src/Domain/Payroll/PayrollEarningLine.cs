...
public sealed class PayrollEarningLine : BaseEntity
{
    private PayrollEarningLine() { }
    private PayrollEarningLine(Guid workerLineId, Guid calculationId, Guid tenantId, Guid farmId, Guid workerId, Guid evidenceId, string type, DateOnly date, Guid attendanceId, long attendanceVersion, DateTimeOffset supervisorVerifiedAt, DateTimeOffset managerConfirmedAt, Guid fieldId, string activitySnapshot, decimal quantity, string unit, string rateType, decimal rate, Guid rateSourceId, long rateVersion, decimal amount, string sourceFingerprint)
    { PayrollWorkerLineId = workerLineId; PayrollCalculationId = calculationId; TenantId = tenantId; FarmId = farmId; WorkerProfileId = workerId; EvidenceId = evidenceId; EvidenceType = type; WorkDate = date; AttendanceId = attendanceId; AttendanceVersion = attendanceVersion; SupervisorVerifiedAtSnapshot = supervisorVerifiedAt; ManagerConfirmedAtSnapshot = managerConfirmedAt; FieldId = fieldId; ActivitySnapshot = activitySnapshot; Quantity = quantity; Unit = unit; RateType = rateType; RateAmountUsd = rate; RateSourceId = rateSourceId; RateVersion = rateVersion; EarningAmountUsd = amount; SourceFingerprint = sourceFingerprint; }
